...
export default function FormFeedBack() {
  ...
  const RateRepId = searchParams.get("RateRepId");
  ...
  const handleCreate = async (value) => {
    ...
    try {
      ...
      formData.append("RateId", value.RateId);
    ...
    try {
      setwaiting(true);
      const response = await axios.put(APILink() + "Rate", formData, {
        headers: {
    ...
    const fetchdata = async () => {
      try {
        ...
        const response = await axios.get(
          APILink() + "Rate/GetByIdRateRep/" + RateRepId,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        form.setFieldsValue({
          FeedBackContent: response.data.data.content,
          Id: response.data.data.id,
          RateId: response.data.data.rateId,
        });
      } catch (error) {
        if (error.response.data === "Invalid email") {
          navigate(`/error`);
  ...
  return (
    <>
      ...
      <div className="container my-5" style={{ height: "auto", width: "70%" }}>
        <h1 className="mb-4">Update Feed Back.</h1>
