...
export default function UpdateFlashSale() {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          APILink() + "FlashSale/GetById/" + flashsaleId,
          {
            headers: {
        ...
        var date = [
          moment(response.data.data.start_Date),
          moment(response.data.data.end_Date),
        ];
        ...
        form.setFieldsValue({
          ...
          Date: date,
          Start: moment(response.data.data.start_Date).format(
            "DD/MM/YYYY HH:mm:ss"
          ),
          End: moment(response.data.data.end_Date).format(
            "DD/MM/YYYY HH:mm:ss"
          ),
        });
  ...
  const handleCreate = async (value) => {
    ...
    try {
      ...
      const response = await axios.put(APILink() + "FlashSale", formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  return (
    <>
      ...
      <div className="container my-5" style={{ height: "auto", width: "50%" }}>
        <h1 className="mb-4">Update Flash Sale.</h1>
