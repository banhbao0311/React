...
export default function FormFeedBack() {
  ...
  const QuestRepId = searchParams.get("QuestRepId");
    ...
    const fetchdata = async () => {
      try {
        ...
        const response = await axios.get(
          APILink() + "Question/GetByIdQues/" + parseInt(QuestRepId),
          {
            headers: {
  ...
  const handleCreate = async (value) => {
    ...
    try {
      ...
      formData.append("QuestionId", value.questionId);
    ...
    try {
      setwaiting(true);
      const response = await axios.put(APILink() + "Question", formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
